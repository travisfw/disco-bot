...
public class CompilationUnit extends ProcessingUnit {
	...
	if(configuration == null || !configuration.getDexOut()){
        addPhaseOperation(output);
	}else{
		JarOutput jo = new JarOutput(configuration);
		addPhaseOperation(jo);
	}
...
//                }
    ...
    */
    ...
    /**
     * This is a variant of the Output operation used in Compilation unit.
     * It writes the classfiles directly to a jar instead of the filesystem.
     */
    public final class JarOutput extends CompilationUnit.GroovyClassOperation {
        private CompilerConfiguration configuration;
        private DexFile dexFile;

        public JarOutput(CompilerConfiguration configuration) {
        ...
        public void call(GroovyClass gclass) throws CompilationFailedException {
            ...
            ClassDefItem clazz = CfTranslator.translate(name, bytes, new CfOptions());
            dexFile.add(clazz);
        }

        @Override
        public void phaseFinished() {
            ZipOutputStream dexOutFile = null;

            try {
                dexOutFile = new ZipOutputStream(new FileOutputStream(new File(configuration.getTargetDirectory(), configuration.getDexName())));
                dexOutFile.putNextEntry(new ZipEntry("classes.dex"));
                byte[] dexBytes = dexFile.toDex(null, false);
                dexOutFile.write(dexBytes, 0, dexBytes.length);
            } catch (Exception e) {
                getErrorCollector().addError(Message.create(e.getMessage(), CompilationUnit.this));
                e.printStackTrace();
            } finally {
